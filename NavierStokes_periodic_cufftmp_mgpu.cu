...
//
// =============================================================================
//  架构 (与你 error-4 旧版本相比的关键改动):
// =============================================================================
//  1. 不使用 cufftXtSetDistribution -- 完全依赖 cuFFTMp 内建 slab 分解。
//     (原 cufftXtSetDistribution 参数顺序错误, 且 upper 应为开区间, 直接放弃。)
//
//  2. Buffer 通过 cufftXtMalloc 分配 (NVSHMEM symmetric heap)。
//     传给 cufftXt* 的 buffer 必须来自 NVSHMEM, 普通 cudaMalloc 指针会崩。
//
//  3. 变换 in-place: V_buf 既存实数 (subFormat=2, X-slab, padded stride 2*NZC)
...
//
//     实数 kernel (X-slab, padded):
//        k  = idx % NZ
//        j  = (idx / NZ) % NY
//        lx = idx / (NY * NZ)
//        gi = r * nx_local + lx
//        pidx = lx * NY * 2*NZC + j * 2*NZC + k
//
//  5. 每次 user kernel (stream 0) 与 cufftXt 调用之间都 cudaDeviceSynchronize()。
//
//  注意: 单节点版本失败 (code 5 = CUFFT_INTERNAL_ERROR) 是 NVSHMEM bootstrap
//        失败, 而不是代码问题。在 sbatch 里 export NVSHMEM_BOOTSTRAP=MPI 即可。
//
...
// ============================================================
// Main (单节点版: GPU = global rank % num_gpus)
// ============================================================
int main(int argc, char** argv) {
    ...
    if (s.rank == 0) {
        cout << "============================================================\n";
        cout << "  Navier-Stokes -- cuFFTMp Built-in Slab (Single Node)\n";
        cout << "============================================================\n";
