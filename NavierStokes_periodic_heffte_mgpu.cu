 ...
 *
 * Run: mpirun -np <ngpu> ./navier_stokes_heffte_mgpu
 *      (ngpu must be a number that supports heFFTe pencil factorization, e.g., 2,4,8)
 *
 * NOTE: If CUDA-aware MPI is unavailable, set HEFFTE_DISABLE_GPU_AWARE_MPI=1 or
 *       use heFFTe built with pack_plan backend (heFFTe will host-stage transfers).
 */
...
// ============================================================
// CUDA kernels (identical to single-GPU version)
// ============================================================
...
// ============================================================
// Physics: nonlinear, rhs, rk4, diagnostics (same as single GPU)
// ============================================================
template<typename FFT>
...
static void compute_rhs(FFT& fft,GPUArrays& g,
    GCplx* r1,GCplx* r2,GCplx* r3,const BoxGPU& br,const BoxGPU& bc,double t)
{
    int gc=bc.grid(BLOCK),gr=br.grid(BLOCK);
