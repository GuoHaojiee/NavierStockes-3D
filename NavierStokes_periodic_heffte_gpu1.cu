...
int main(int argc, char** argv) {
    ...
    // ---- Initial diagnostics ----
    {
        auto [e0, d0] = compute_diagnostics(fft, g, br, bc, 0.);
        if (rank==0) {
            cout << "\nInitial (t=0):\n";
            cout << "  L2 error:   " << scientific << e0 << "\n";
            cout << "  max|div V|: " << d0 << "\n\n";
        }
    }
    ...
    for (int it=0; it<=NT_RUN; ++it) {
        ...
        if (rank==0)
            cout << setw(6)<<it << setw(12)<<fixed<<setprecision(4)<<t_wall
                 << setw(16)<<scientific<<setprecision(4)<<e
                 << setw(16)<<d << "\n";
        if (it<NT_RUN) {
