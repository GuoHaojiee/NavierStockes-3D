...
int main(int argc, char** argv) {
    ...
    // =========================================================================
    // ★ FIX: bind to correct GPU BEFORE any cudaMemcpyToSymbol.
    // cudaMemcpyToSymbol writes to the *current* device's constant memory.
    // All ranks default to device 0, so ranks 1..P-1 would write NX/NY/NZ to
    // GPU 0, leaving their own GPU's d_NX/d_NY/d_NZC = 0.  Kernels on those
    // ranks then compute n_padded = nx_local * 0 * 0 = 0 and return immediately,
    // leaving the buffer all-zeros.  cuFFTMp sees correct data only from rank 0,
    // giving amplitude = nx_local/(2*NX) = 0.125 instead of 0.5.
    // =========================================================================
    MPI_Comm node_comm;
    ...
    // Fix1+Fix2: SetSubformatDefault MUST precede MakePlan3d.
    ...
    // 决定性实验：alloc 顺序消解
    //
    // 实验 A: 只 alloc V1_buf，立刻 probe → 看 idx=4160 的值
    // 实验 B: alloc 完所有 9 个 FFT buffer，再 probe V1_buf → 对比
    //
    // 预期结果矩阵：
    //   A=0.5  B=0.5  → alloc 顺序不是 culprit
    //   A=0.5  B=0.125 → 多 buffer alloc 污染了 V1_buf 的 transpose metadata ← 强假说
    //   A=0.125 B=0.125 → culprit 不在 alloc，还没找到
    // =========================================================================

    // 先 alloc scratch（probe_layout 内部需要）
    {
        long long sc_size = std::max(s.nc_local, s.nr_local);
        CUDA_CHECK(cudaMalloc(&s.scratch, sc_size * sizeof(double)));
        CUDA_CHECK(cudaMemset(s.scratch, 0, sc_size * sizeof(double)));
    ...
    // -----------------------------------------------------------------
    // 实验 A：只 alloc V1_buf，probe 它
    // -----------------------------------------------------------------
    alloc_and_zero(plan_r2c, &s.V1_buf, CUFFT_XT_FORMAT_INPLACE);

    if (s.rank == 0) {
        cout << "\n========================================\n"
             << "[EXPERIMENT A] Only V1_buf alloc'd\n"
             << "  Hypothesis: V1_buf alone => correct transpose (0.5)\n"
             << "========================================\n" << flush;
    }
    probe_layout(plan_r2c, s, 2, "PROBE-A (1 buf alloc'd)");

    // -----------------------------------------------------------------
    // 实验 B：alloc 剩余 8 个 FFT buffer，再 probe V1_buf
    // -----------------------------------------------------------------
    alloc_and_zero(plan_r2c, &s.V2_buf,    CUFFT_XT_FORMAT_INPLACE);
    alloc_and_zero(plan_r2c, &s.V3_buf,    CUFFT_XT_FORMAT_INPLACE);
    alloc_and_zero(plan_r2c, &s.rot1_buf,  CUFFT_XT_FORMAT_INPLACE);
    ...
    if (s.rank == 0) {
        cout << "\n========================================\n"
             << "[EXPERIMENT B] All 9 FFT buffers alloc'd, probing V1_buf again\n"
             << "  Hypothesis: extra allocs corrupt V1_buf metadata => 0.125\n"
             << "========================================\n" << flush;
    }
    probe_layout(plan_r2c, s, 2, "PROBE-B (9 bufs alloc'd)");

    // -----------------------------------------------------------------
    // 实验 C：probe 一个其他 buffer（V2_buf），对比同等条件下的行为
    // -----------------------------------------------------------------
    // 把 cos(2x) 填入 V2_buf，FFT，看 V2_buf 的 idx=4160
    {
        if (s.rank == 0) {
            cout << "\n========================================\n"
                 << "[EXPERIMENT C] Probe V2_buf (freshly alloc'd sibling)\n"
                 << "  If V2_buf=0.5 but V1_buf=0.125, V1_buf specifically is corrupted\n"
                 << "  If V2_buf=0.125 too, all bufs after first are corrupted\n"
                 << "========================================\n" << flush;
        }
        ...
        // Fill V2_buf with cos(2π·2·gi/NX)
        CUDA_CHECK(cudaMemset(gpu_ptr(s.V2_buf), 0, gpu_size_bytes(s.V2_buf)));
        kernel_probe_fill_cosX<<<gp, BLOCK>>>(
            (double*)gpu_ptr(s.V2_buf), s.nx_local, s.x_offset, 2);
        CUDA_CHECK(cudaDeviceSynchronize());
        ...
        kernel_scale_cplx<<<gc, BLOCK>>>(
            (GCplx*)gpu_ptr(s.V2_buf), s.nc_local, inv_N);
        CUDA_CHECK(cudaDeviceSynchronize());

        // Copy to host and scan
        std::vector<GCplx> host(s.nc_local);
        CUDA_CHECK(cudaMemcpy(host.data(), gpu_ptr(s.V2_buf),
                              s.nc_local * sizeof(GCplx), cudaMemcpyDeviceToHost));

        // Compute expected indices for Layout A, KX0=2
        long long predA_pos = (long long)2        * s.ny_local * NZC;  // idx=4160
        long long predA_neg = (long long)(NX - 2) * s.ny_local * NZC;  // idx=262080

        MPI_Barrier(MPI_COMM_WORLD);
        for (int r = 0; r < s.nprocs; ++r) {
            if (r == s.rank) {
                printf("--- PROBE-C rank %d  y_offset=%d ---\n", s.rank, s.y_offset);
                if (s.rank == 0) {
                    // Print expected entries
                    if (predA_pos < s.nc_local) {
                        double mag = sqrt(host[predA_pos].x*host[predA_pos].x +
                                          host[predA_pos].y*host[predA_pos].y);
                        printf("  [LayoutA pos] idx=%lld  V=(%+.8f,%+.8f)  |V|=%.8f  %s\n",
                               predA_pos, host[predA_pos].x, host[predA_pos].y, mag,
                               fabs(mag-0.5)<1e-4 ? "✓ 0.5 CORRECT" :
                               fabs(mag-0.125)<1e-4 ? "✗ 0.125 PARTIAL-SUM" : "✗ unexpected");
                    }
                    if (predA_neg < s.nc_local) {
                        double mag = sqrt(host[predA_neg].x*host[predA_neg].x +
                                          host[predA_neg].y*host[predA_neg].y);
                        printf("  [LayoutA neg] idx=%lld  V=(%+.8f,%+.8f)  |V|=%.8f\n",
                               predA_neg, host[predA_neg].x, host[predA_neg].y, mag);
                    }
                }
                // Count leak entries
                int leak_count = 0; double leak_max = 0.0;
                for (long long i = 0; i < s.nc_local; ++i) {
                    if (i == predA_pos || i == predA_neg) continue;
                    double mag = sqrt(host[i].x*host[i].x + host[i].y*host[i].y);
                    if (mag > 1e-6) { ++leak_count; leak_max = std::max(leak_max, mag); }
                }
                printf("  rank %d: leak_count=%d  leak_max=%.4e\n",
                       s.rank, leak_count, leak_max);
                fflush(stdout);
            }
            MPI_Barrier(MPI_COMM_WORLD);
        }

        // Zero V2_buf after probe
        CUDA_CHECK(cudaMemset(gpu_ptr(s.V2_buf), 0, gpu_size_bytes(s.V2_buf)));
        CUDA_CHECK(cudaDeviceSynchronize());
    }
    ...
    if (s.rank == 0) {
        cout << "\n========================================\n"
             << "  RESULT MATRIX:\n"
             << "  A=0.5  B=0.5  C=0.5  → alloc order NOT culprit\n"
             << "  A=0.5  B=0.125 C=0.125 → extra allocs corrupt ALL bufs\n"
             << "  A=0.5  B=0.125 C=0.5  → V1_buf specifically corrupted by extra allocs\n"
             << "  A=0.125 B=0.125 C=0.125 → not alloc, something else\n"
             << "========================================\n" << flush;
    }

    // Alloc remaining plain-cudaMalloc state (needed for free_state)
    {
        auto C = [&](GCplx** p){
            CUDA_CHECK(cudaMalloc(p, s.nc_local*sizeof(GCplx)));
            CUDA_CHECK(cudaMemset(*p, 0, s.nc_local*sizeof(GCplx)));
