...
__global__ void kernel_compute_rot(const GCplx* V1, const GCplx* V2, const GCplx* V3,
    ...
    long long idx = (long long)blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= nc_local) return;
    // Layout A: cuFFTMp INPLACE_SHUFFLED = [gx][local_y][kz]
    // stride: gx-step = ny_local*NZC, ly-step = NZC, kz-step = 1
...
int main(int argc, char** argv) {
    ...
    // Fix 1+2: SetSubformatDefault MUST come before MakePlan3d.
    // Tells cuFFTMp which buffer layout to expect on each FFT call,
    // so the internal transpose plan is built correctly at plan creation time.
    CUFFT_CHECK(cufftXtSetSubformatDefault(plan_r2c,
        CUFFT_XT_FORMAT_INPLACE,           // D2Z input:  X-slab real
        CUFFT_XT_FORMAT_INPLACE_SHUFFLED)); // D2Z output: Y-slab complex
    CUFFT_CHECK(cufftXtSetSubformatDefault(plan_c2r,
        CUFFT_XT_FORMAT_INPLACE_SHUFFLED,  // Z2D input:  Y-slab complex
        CUFFT_XT_FORMAT_INPLACE));          // Z2D output: X-slab real
